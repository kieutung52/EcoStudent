...
@section('title', 'Sửa bài viết - EcoStudent')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <h1 class="text-3xl font-bold mb-2">Sửa bài viết</h1>
        <p class="text-gray-600">Chỉnh sửa thông tin bài viết và sản phẩm của bạn</p>
    </div>
        ...
        <form id="edit-post-form" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea id="content" 
                          name="content" 
                          rows="4"
                          class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $post->content }}</textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="university_id" 
                    ...
                    @foreach($universities as $university)
                        <option value="{{ $university->id }}" {{ $post->university_id == $university->id ? 'selected' : '' }}>
                            {{ $university->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-6">
                ...
                <div id="products-container" class="space-y-4">
                    @foreach($post->products as $index => $product)
                        <div class="product-item border rounded-lg p-4 bg-gray-50" data-product-id="{{ $product->id }}">
                            <div class="flex justify-between items-center mb-4">
                                <h3 class="font-semibold text-gray-800">Sản phẩm {{ $index + 1 }}</h3>
...
@section('scripts')
<script>
let productCount = {{ $post->products->count() }};
...
// Remove product button handlers
...
document.getElementById('edit-post-form').addEventListener('submit', async function(e) {
    ...
    if (!token) {
        alert('Vui lòng đăng nhập để sửa bài');
